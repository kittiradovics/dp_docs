...
class APIPathParamTypeForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => '\Drupal\dp_docs\Entity\APIPathParamType::load',
      ],
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        drupal_set_message($this->t('Created the %label API HTTP Method Path Parameter type.', [
          '%label' => $api_path_param_type->label(),
        ]));
        break;

      default:
        drupal_set_message($this->t('Saved the %label API HTTP Method Path Parameter type.', [
          '%label' => $api_path_param_type->label(),
        ]));
    }
