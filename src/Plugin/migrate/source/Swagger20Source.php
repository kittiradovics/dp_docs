...
namespace Drupal\dp_docs\Plugin\migrate\source;
...
abstract class Swagger20Source extends SourcePluginBase {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, MigrationInterface $migration) {
    ...
    foreach ($source_plugin_manager->getDefinitions() as $definition) {
      if (in_array('Drupal\dp_docs\Plugin\migrate\source\Swagger20SourceInterface', class_implements($definition['class']))) {
  ...
  public function getSourceData() {
    $data = [];
    $data_cid = "dp_docs:source_file:{$this->sourceFile}";
  ...
  protected function setSourceNodeKeyRec(array &$node, $field_name, $path, $parent_source_key) {
    /** @var \Drupal\dp_docs\Plugin\migrate\source\Swagger20SourceInterface $class */
