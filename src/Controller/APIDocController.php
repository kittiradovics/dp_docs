...
use Drupal\dp_docs\APIDocStorageInterface;

/**
 * Returns responses for API Doc routes.
 *
...
class APIDocController extends ControllerBase {
  ...
  /**
   * Constructs an APIDocController object.
   *
  ...
  /**
   * Displays an API Doc revision.
   *
   * @param int $api_doc_revision
   *   The API Doc revision ID.
   *
   * @return array
  ...
  public function revisionShow($api_doc_revision) {
    ...
    $view_builder = $this->entityTypeManager()->getViewBuilder('api_doc');
    return $view_builder->view($api_doc);
  }

  /**
   * Page title callback for an API Doc revision.
   *
   * @param int $api_doc_revision
   *   The API Doc revision ID.
   *
  ...
  public function revisionPageTitle($api_doc_revision) {
    ...
    $api_doc = $this->entityTypeManager()->getStorage('api_doc')->loadRevision($api_doc_revision);
    return $this->t('Revision of %title from %date', ['%title' => $api_doc->label(), '%date' => $this->dateFormatter->format($api_doc->getRevisionCreationTime())]);
  }

  /**
   * Generates an overview table of older revisions of a API Doc.
   *
   * @param \Drupal\dp_docs\APIDocInterface $api_doc
   *   An API Doc object.
   *
   * @return array
  ...
  public function revisionOverview(APIDocInterface $api_doc) {
    ...
    $langname = $api_doc->language()->getName();
    $languages = $api_doc->getTranslationLanguages();
    $has_translations = (count($languages) > 1);
    /** @var \Drupal\dp_docs\APIDocStorageInterface $api_doc_storage */
    $api_doc_storage = $this->entityTypeManager()->getStorage('api_doc');

    $build['#title'] = $has_translations ? $this->t('@langname revisions for %title', ['@langname' => $langname, '%title' => $api_doc->label()]) : $this->t('Revisions for %title', ['%title' => $api_doc->label()]);
    $header = [$this->t('Revision'), $this->t('Operations')];

    $revert_permission = ($account->hasPermission("revert all api doc revisions") || $account->hasPermission('administer api docs'));
    $delete_permission = ($account->hasPermission("delete all api doc revisions") || $account->hasPermission('administer api docs'));

    $rows = [];
    $default_revision = $api_doc->getRevisionId();

    foreach ($this->getRevisionIds($api_doc, $api_doc_storage) as $vid) {
      /** @var \Drupal\dp_docs\APIDocInterface $revision */
      $revision = $api_doc_storage->loadRevision($vid);
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($vid != $api_doc->getRevisionId()) {
          $link = Link::fromTextAndUrl($date, new Url('entity.api_doc.revision', ['api_doc' => $api_doc->id(), 'api_doc_revision' => $vid]))->toString();
        }
        else {
          $link = $api_doc->toLink($date)->toString();
        }
        ...
        if ($vid == $default_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              'title' => $vid < $api_doc->getRevisionId() ? $this->t('Revert') : $this->t('Set as current revision'),
              'url' => $has_translations ?
                Url::fromRoute('entity.api_doc.translation_revert', ['api_doc' => $api_doc->id(), 'api_doc_revision' => $vid, 'langcode' => $langcode]) :
                Url::fromRoute('entity.api_doc.revision_revert', ['api_doc' => $api_doc->id(), 'api_doc_revision' => $vid]),
            ];
          }
          ...
          if ($delete_permission) {
            $links['delete'] = [
              'title' => $this->t('Delete'),
              'url' => Url::fromRoute('entity.api_doc.revision_delete', ['api_doc' => $api_doc->id(), 'api_doc_revision' => $vid]),
            ];
          }
    ...
    $build['api_doc_revisions_table'] = [
      '#theme' => 'table',
      '#rows' => $rows,
      '#header' => $header,
      '#attributes' => ['class' => 'api-doc-revision-table'],
    ];
   ...
   * @param \Drupal\dp_docs\APIDocInterface $api_doc
   *   The API Doc entity.
   * @param \Drupal\dp_docs\APIDocStorageInterface $api_doc_storage
   *   The API Doc storage handler.
   *
   * @return int[]
   *   API Doc revision IDs (in descending order).
   */
  protected function getRevisionIds(APIDocInterface $api_doc, APIDocStorageInterface $api_doc_storage) {
    $result = $api_doc_storage->getQuery()
      ->allRevisions()
      ->condition($api_doc->getEntityType()->getKey('id'), $api_doc->id())
      ->sort($api_doc->getEntityType()->getKey('revision'), 'DESC')
      ->pager(50)
      ->execute();
