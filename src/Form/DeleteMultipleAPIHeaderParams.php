...
class DeleteMultipleAPIHeaderParams extends ConfirmFormBase {
   ...
   *
   * @var \Drupal\dp_docs\APIHeaderParamStorageInterface
   */
  protected $storage;
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    /** @var \Drupal\dp_docs\APIHeaderParamInterface[] $api_header_params */
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if ($form_state->getValue('confirm') && !empty($this->apiHeaderParamInfo)) {
      ...
      if ($total_count) {
        drupal_set_message($this->formatPlural($total_count, 'Deleted 1 API HTTP Method Header Parameter.', 'Deleted @count API HTTP Method Header Parameters.'));
      }
