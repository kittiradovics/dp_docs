...
class DeleteMultipleAPIBodyParams extends ConfirmFormBase {
   ...
   *
   * @var \Drupal\dp_docs\APIBodyParamStorageInterface
   */
  protected $storage;
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    /** @var \Drupal\dp_docs\APIBodyParamInterface[] $api_body_params */
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if ($form_state->getValue('confirm') && !empty($this->apiBodyParamInfo)) {
      ...
      if ($total_count) {
        drupal_set_message($this->formatPlural($total_count, 'Deleted 1 API HTTP Method Body Parameter.', 'Deleted @count API HTTP Method Body Parameters.'));
      }
