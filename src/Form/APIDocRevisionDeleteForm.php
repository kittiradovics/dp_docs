...
/**
 * Provides a form for deleting an API Doc revision.
 *
 * @ingroup dp_docs
 */
class APIDocRevisionDeleteForm extends ConfirmFormBase {

  /**
   * The API Doc revision.
   *
   * @var \Drupal\dp_docs\APIDocInterface
   */
  protected $revision;
  ...
  /**
   * Constructs a new APIDocRevisionDeleteForm.
   *
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('api_doc'),
      $connection,
      $date_formatter
    );
  }
  ...
  public function getFormId() {
    return 'api_doc_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->apiDocStorage->deleteRevision($this->revision->getRevisionId());

    $this->logger('content')->notice('API Documentation: deleted %title revision %revision.', [
      '%title' => $this->revision->label(),
      '%revision' => $this->revision->getRevisionId(),
    ]);
    drupal_set_message(t('Revision from %revision-date of API Documentation %title has been deleted.', [
      '%revision-date' => $this->dateFormatter->format($this->revision->getRevisionCreationTime()),
      '%title' => $this->revision->label(),
    ]));
    $form_state->setRedirect(
      'entity.api_doc.canonical',
       ['api_doc' => $this->revision->id()]
    );
    $query = $this->connection->select('api_doc_field_revision');
