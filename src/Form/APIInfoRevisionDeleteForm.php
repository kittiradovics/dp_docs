...
/**
 * Provides a form for deleting an API Info revision.
 *
 * @ingroup dp_docs
 */
class APIInfoRevisionDeleteForm extends ConfirmFormBase {

  /**
   * The API Info revision.
   *
   * @var \Drupal\dp_docs\APIInfoInterface
   */
  protected $revision;

  /**
   * The API Info storage.
   *
  ...
  /**
   * Constructs a new APIInfoRevisionDeleteForm.
   *
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('api_info'),
      $connection,
      $date_formatter
    );
  }
  ...
  public function getFormId() {
    return 'api_info_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->apiInfoStorage->deleteRevision($this->revision->getRevisionId());

    $this->logger('content')->notice('API Info: deleted %title revision %revision.', [
      '%title' => $this->revision->label(),
      '%revision' => $this->revision->getRevisionId(),
    ]);
    drupal_set_message(t('Revision from %revision-date of API Info %title has been deleted.', [
      '%revision-date' => $this->dateFormatter->format($this->revision->getRevisionCreationTime()),
      '%title' => $this->revision->label(),
    ]));
    $form_state->setRedirect(
      'entity.api_info.canonical',
       ['api_info' => $this->revision->id()]
    );
    $query = $this->connection->select('api_info_field_revision');
