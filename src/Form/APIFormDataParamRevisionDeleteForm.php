...
/**
 * Provides a form for deleting an API Form Data Param revision.
 *
 * @ingroup dp_docs
 */
class APIFormDataParamRevisionDeleteForm extends ConfirmFormBase {

  /**
   * The API Form Data Param revision.
   *
  ...
  /**
   * The API Form Data Param storage.
   *
  ...
  /**
   * Constructs a new APIFormDataParamRevisionDeleteForm.
   *
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('api_form_data_param'),
      $connection,
      $date_formatter
    );
  }
  ...
  public function getFormId() {
    return 'api_form_data_param_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->apiFormDataParamStorage->deleteRevision($this->revision->getRevisionId());

    $this->logger('content')->notice('API HTTP Method Form Data Parameter: deleted %title revision %revision.', [
      '%title' => $this->revision->label(),
      '%revision' => $this->revision->getRevisionId(),
    ]);
    drupal_set_message(t('Revision from %revision-date of API HTTP Method Form Data Parameter %title has been deleted.', [
      '%revision-date' => $this->dateFormatter->format($this->revision->getRevisionCreationTime()),
      '%title' => $this->revision->label(),
    ]));
    $form_state->setRedirect(
      'entity.api_form_data_param.canonical',
       ['api_form_data_param' => $this->revision->id()]
    );
    $query = $this->connection->select('api_form_data_param_field_revision');
